...
st.markdown(
    "<h1 style='text-align: center; color: #1f77b4;'>OP & Sales vs Stock Price Analyzer</h1>",
    unsafe_allow_html=True
)
st.markdown(
    "<p style='text-align: center; font-size: 16px; color: #666;'>"    
    "<strong style='color: #2ca02c;'>P&L saved locally — no re-scrape!</strong></p>",
    unsafe_allow_html=True
)
...
# LOAD OR SCRAPE P&L
# ───────────────────────────────────────────────
def get_pl_data(ticker: str, force_scrape: bool = False) -> pd.DataFrame | None:
    ...
    with st.spinner(f"Scraping P&L for **{ticker}**..."):
        df = scrape_screener_data(ticker)
        if df is not None:
            df.to_csv(file_path)
...
# CORE ANALYSIS
# ───────────────────────────────────────────────
def analyze_single_ticker(ticker: str, fy_start: int, force_scrape: bool = False):
    ticker = ticker.strip().upper()
    pl_df = get_pl_data(ticker, force_scrape)
...
if st.sidebar.button("Analyze All", type="primary"):
    ...
    for i, t in enumerate(tickers):
        with st.spinner(f"Analyzing {t}..."):
            res = analyze_single_ticker(t, fy_start, force_scrape)
            results.append(res)
...
st.caption(f"Data cached in `{DATA_DIR}/` • Re-scrape with button above")