...
st.markdown("Upload a CSV file with stock symbols and type (v200, v40, v40next, ML) to screen stocks based on Simple Moving Averages (20, 50, 200 days) and V20 strategy. Stocks with current price within 2% of V20 strategy low price are highlighted.")
    ...
    for index, row in symbols_df.iterrows():
        ...
        try:
            ...
            if stock_type in ['v200', 'v40', 'v40next']:
                ...
                if momentum_gain >= 20:
                    # Check if current price is within 2% of V20 low price
...
if st.button("Screen Stocks"):
    if not symbols_df.empty:
        ...
        if not v20_df.empty:
            # Highlight stocks where current price is within 2% of V20 low price
            def highlight_near_v20_low(row):
                if row['Near V20 Low (Within 2%)'] == 'Yes':
                    return ['background-color: #90EE90'] * len(row)  # Light green background
                return [''] * len(row)
            
            st.dataframe(v20_df.style.apply(highlight_near_v20_low, axis=1))
