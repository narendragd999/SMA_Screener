...
# ---------- NEW: Quarterly scraping, parsing & TTM helpers ----------
def parse_quarterly_table(table) -> pd.DataFrame | None:
    """Parse Screener quarterly results table into tidy DataFrame with Quarter index (datetime)."""
    try:
        thead = table.find("thead")
        if not thead:
            return None
        ...
        if not rows:
            return None
        qdf = pd.DataFrame(rows, columns=headers).set_index(headers[0])
        # Convert quarter columns to datetimes where possible; keep TTM column if present
        col_map = {}
        parsed_cols = []
        for c in qdf.columns:
            ctxt = c.strip()
            if ctxt.upper() == "TTM":
                parsed_cols.append("TTM")
                col_map[c] = "TTM"
                continue
            # Try parsing like 'Sep 2024', 'Mar 2025'
            d = pd.to_datetime(ctxt, format="%b %Y", errors="coerce")
            if pd.isna(d):
                d = pd.to_datetime(ctxt, errors="coerce")
            if not pd.isna(d):
                parsed_cols.append(d)
                col_map[c] = d
            else:
                parsed_cols.append(ctxt)
                col_map[c] = ctxt
        qdf = qdf.rename(columns=col_map)
        # Order datetime-like columns first then others
        datetime_cols = [c for c in qdf.columns if isinstance(c, (pd.Timestamp, datetime.date))]
        datetime_cols = sorted(datetime_cols)
        rest = [c for c in qdf.columns if c not in datetime_cols]
        ordered = datetime_cols + rest
        qdf = qdf[ordered]
        # Clean numeric values across the DataFrame
        qdf_clean = qdf.applymap(lambda x: str(x).strip()).replace("", "0")
        for col in qdf_clean.columns:
            qdf_clean[col] = (
                qdf_clean[col]
                .astype(str)
        ...
        qdf_clean.index.name = "Metric"
        return qdf_clean
    except Exception as e:
        if st.session_state.get("debug_mode", False):
            st.error(f"Quarterly parse error: {e}")
        return None


def scrape_screener_quarters(ticker: str, debug: bool = False) -> pd.DataFrame | None:
    """
    Scrape Screener 'Quarterly Results' table for ticker (consolidated).
    Returns parsed DataFrame (metrics x quarters).
    """
    ticker = ticker.strip().upper()
    urls = [
        f"https://www.screener.in/company/{ticker}/consolidated/",
    ...
    for url in urls:
        ...
        soup = BeautifulSoup(html, "html.parser")
        # Try common headings
        q_header = soup.find(lambda tag: tag.name in ["h2", "h3"] and "Quarterly" in tag.get_text())
        table = None
        if q_header:
            table = q_header.find_next("table")
        # fallback: search for table with quarter-like headers
        if table is None:
            ...
            for t in tables:
                ...
                if any(m in header_text for m in ["JAN", "FEB", "MAR", "APR", "MAY", "JUN", "JUL", "AUG", "SEP", "OCT", "NOV", "DEC", "TTM"]):
                    table = t
                    break
        if table is None:
            continue
        qdf = parse_quarterly_table(table)
        if qdf is not None and not qdf.empty:
            return qdf
    return None


def compute_ttm(quarterly_df: pd.DataFrame, metric_names: list = None) -> pd.DataFrame:
    """
    Compute TTM series for provided metrics from a quarterly DataFrame.
    quarterly_df expected columns ordered oldest->newest and numeric.
    Returns DataFrame indexed by the quarter (column name of the 'ending quarter') with TTM sums.
    """
    cols = [c for c in quarterly_df.columns if c != "TTM"]
    # Ensure chronological order if datetime-like columns
    try:
        datetime_cols = [c for c in cols if isinstance(c, (pd.Timestamp, datetime.date))]
        datetime_cols = sorted(datetime_cols)
        rest = [c for c in cols if c not in datetime_cols]
        cols = datetime_cols + rest if datetime_cols else cols
    except Exception:
        pass
    ttm_rows = {}
    for i in range(3, len(cols)):
        window = cols[i-3:i+1]
        q_label = cols[i]  # use last quarter in window as label
        sums = quarterly_df[window].sum(axis=1)
        ttm_rows[q_label] = sums
    ttm_df = pd.DataFrame(ttm_rows).T
    ttm_df = ttm_df.fillna(0)
    return ttm_df


def build_ttm_model(ttm_df: pd.DataFrame, price_quarterly: pd.Series, metric_name: str = "Operating Profit") -> dict:
    """
    Build simple linear model for TTM metric -> avg price.
    price_quarterly: Series indexed by the same quarter labels used by ttm_df (matching dtype).
    metric_name: metric row name in ttm_df to use as predictor.
    Returns dict with model, r2, eq, latest_ttm, pred_price, etc.
    """
    out = {"model": None, "r2": None, "eq": None, "pred_price": None, "latest_ttm": None, "index": []}
    if metric_name not in ttm_df.columns:
        return out
    common = ttm_df.index.intersection(price_quarterly.index)
    if len(common) < 2:
        return out
    X = ttm_df.loc[common, metric_name].astype(float).values.reshape(-1, 1)
    y = price_quarterly.loc[common].astype(float).values
    try:
        lr = LinearRegression().fit(X, y)
        latest = float(ttm_df.loc[common, metric_name].iloc[-1])
        pred = float(lr.predict([[latest]])[0])
        out.update({
            "model": lr,
            "r2": round(lr.score(X, y), 3),
            "eq": f"Price = {round(lr.intercept_,2)} + {round(lr.coef_[0],6)} × {metric_name}",
        ...
        if st.session_state.get("debug_mode", False):
            st.error(f"TTM model build error: {e}")
    return out
# ---------- END NEW HELPERS ----------
...
def analyze_single_ticker(ticker: str, fy_start: int, force_scrape: bool = False, debug: bool = False, include_other_income: bool = False):
    ...
    # ----------------- NEW: Quarterly & TTM processing -----------------
    quarterly_df = None
    ttm_df = None
    ttm_model_info = None
    try:
        qdf = scrape_screener_quarters(ticker, debug=debug)
        if qdf is not None and not qdf.empty:
            # Normalize metric names
            qdf_index_lower = [str(i).lower() for i in qdf.index]
            op_idx = None
            for candidate in ["operating profit", "operatingprofit", "op", "ebit", "ebitda"]:
                for i, name in enumerate(qdf_index_lower):
                ...
                if op_idx:
                    break
            sales_idx = None
            for candidate in ["sales", "net sales", "revenue", "total income"]:
                for i, name in enumerate(qdf_index_lower):
                    if candidate in name:
                        sales_idx = qdf.index[i]
                        break
                if sales_idx:
                    break
            # select a few useful rows
            metrics_to_keep = []
            for key in [sales_idx, op_idx]:
                if key is not None and key in qdf.index:
                    metrics_to_keep.append(key)
            # also try to include Net Profit / Profit Before Tax if present
            for idx_name in qdf.index:
                lname = str(idx_name).lower()
                if "net profit" in lname or "profit before tax" in lname:
                    metrics_to_keep.append(idx_name)
            if not metrics_to_keep:
                # fallback: keep first 5 rows
                metrics_to_keep = list(qdf.index[:5])
            quarterly_df = qdf.loc[metrics_to_keep].copy()
            # Compute TTM
            ttm_df = compute_ttm(quarterly_df)
            # Align price to quarters: compute avg price per quarter (use quarter end)
            price_q = price_df.copy()
            price_q["QuarterEnd"] = price_q.index.to_period("Q").end_time
            price_q_group = price_q.groupby("QuarterEnd")["Close"].mean()
            # prepare price_quarterly Series indexed similar to ttm_df.index
            price_quarterly = pd.Series(dtype=float)
            try:
                # convert price_q_group index to datetime (it typically already is)
                price_q_group.index = pd.to_datetime(price_q_group.index)
                # build a mapping from ttm index to price by matching year-month
                mapping = {}
                for q in ttm_df.index:
                    mapped_q = None
                    if isinstance(q, (pd.Timestamp, datetime.date)):
                        mapped_q = pd.to_datetime(q)
                    else:
                        try:
                            mapped_q = pd.to_datetime(str(q), errors="coerce")
                        except:
                            mapped_q = None
                    if mapped_q is not None:
                        # find exact match in price_q_group (quarter-end)
                        if mapped_q in price_q_group.index:
                            mapping[q] = price_q_group.loc[mapped_q]
                        else:
                            # try match by year & quarter
                            period = mapped_q.to_period("Q")
                            pe = period.end_time
                            if pe in price_q_group.index:
                                mapping[q] = price_q_group.loc[pe]
                if mapping:
                    price_quarterly = pd.Series(mapping)
            except Exception:
                price_quarterly = pd.Series(dtype=float)
            # Build TTM model (use Operating Profit row if found)
            metric_for_model = None
            if op_idx and op_idx in ttm_df.columns:
                metric_for_model = op_idx
            else:
                for col in ttm_df.columns:
                    if "operating" in str(col).lower() or "profit" in str(col).lower():
                        metric_for_model = col
                        break
            if metric_for_model is not None and not price_quarterly.empty:
                ttm_model_info = build_ttm_model(ttm_df, price_quarterly, metric_name=metric_for_model)
    except Exception as e:
        if st.session_state.get("debug_mode", False):
            st.error(f"TTM processing error: {e}")
    # Attach to output will be added to return dict below
...
if "summary_df" in st.session_state:
    ...
    if selected and "analysis_results" in st.session_state and selected in st.session_state.analysis_results:
        ...
        csv = combined.to_csv().encode()
                # ---------- NEW: Quarterly & TTM Analysis Tab ----------
        st.markdown("## Quarterly & TTM Analysis")
        with st.expander("Open Quarterly & TTM Analysis", expanded=True):
            qdf = res.get("quarterly_df")
            ttm = res.get("ttm_df")
            ttm_model = res.get("ttm_model_info")

            if qdf is None or (hasattr(qdf, "empty") and qdf.empty):
                st.info("No Quarterly Results table found on Screener for this ticker.")
            else:
                st.markdown("### Quarterly P&L (extracted from Screener)")
                display_q = qdf.copy()
                display_q.index = display_q.index.astype(str)
                st.dataframe(display_q.T, use_container_width=True)

                if ttm is not None and not ttm.empty:
                    st.markdown("### Trailing Twelve Months (TTM) — computed from last 4 quarters")
                    last_ttm = ttm.tail(8)
                    df_ttm_display = last_ttm.copy()
                    df_ttm_display.index = df_ttm_display.index.astype(str)
                    st.dataframe(df_ttm_display, use_container_width=True)

                    # Chart: TTM metric vs Avg Price (if model exists)
                    try:
                        if isinstance(ttm_model, dict) and ttm_model.get("index"):
                            # Build merged plot table
                            price_series = res.get("price_series")
                            if price_series is not None:
                                price_series["Date"] = pd.to_datetime(price_series["Date"])
                                price_series["QuarterEnd"] = price_series["Date"].dt.to_period("Q").dt.end_time
                                price_q_mean = price_series.groupby("QuarterEnd")["Close"].mean().reset_index()
                                price_q_mean["QuarterEnd"] = pd.to_datetime(price_q_mean["QuarterEnd"])
                                # Prepare ttm plot df
                                try:
                                    ttm_plot_df = ttm.loc[ttm_model["index"]].reset_index()
                                    ttm_plot_df.columns = ["QuarterEnd"] + list(ttm_plot_df.columns[1:])
                                    merged_ttm_plot = pd.merge(ttm_plot_df, price_q_mean, left_on="QuarterEnd", right_on="QuarterEnd", how="left")
                                    metric_cols = [c for c in merged_ttm_plot.columns if c not in ["QuarterEnd","Close"]]
                                    profit_col = None
                                    for c in metric_cols:
                                        if "operating" in str(c).lower() or "profit" in str(c).lower():
                                            profit_col = c
                                            break
                                    if profit_col is None and metric_cols:
                                        profit_col = metric_cols[0]
                                    if profit_col:
                                        chart_ttm = alt.Chart(merged_ttm_plot).encode(x=alt.X("QuarterEnd:T", title="Quarter"))
                                        line_ttm = chart_ttm.mark_line().encode(y=alt.Y(f"{profit_col}:Q", title=f"{profit_col}"), tooltip=[alt.Tooltip("QuarterEnd:T"), alt.Tooltip(f"{profit_col}:Q")]).properties()
                                        line_price = chart_ttm.mark_line().encode(y=alt.Y("Close:Q", title="Avg Price (Rs)"), tooltip=[alt.Tooltip("QuarterEnd:T"), alt.Tooltip("Close:Q")])
                                        combined = alt.layer(line_ttm, line_price).resolve_scale(y="independent").properties(height=360)
                                        st.altair_chart(combined, use_container_width=True)
                                except Exception as e:
                                    if st.session_state.get("debug_mode", False):
                                        st.error(f"TTM chart merge error: {e}")
                            else:
                                st.info("Price series not available to align with TTM.")
                    except Exception as e:
                        if st.session_state.get("debug_mode", False):
                            st.error(f"TTM plotting error: {e}")

                    if ttm_model and isinstance(ttm_model, dict):
                        st.markdown("### TTM Model Summary")
                        if ttm_model.get("r2") is not None:
                            st.write(f"Equation: `{ttm_model.get('eq')}`")
                            st.write(f"R²: `{ttm_model.get('r2')}`")
                            st.metric("Latest TTM value", f\"{ttm_model.get('latest_ttm'):,}\")
                            st.metric("Predicted Price from Latest TTM", f\"Rs.{ttm_model.get('pred_price'):,}\")
                        else:
                            st.info("No TTM model could be fit (not enough overlapping data).")
                else:
                    st.info("Not enough quarterly data to compute TTM.")

                # Download combined CSV
                try:
                    combined_dl = quarterly_df.copy()
                    if ttm is not None and not ttm.empty:
                        ttm_display = ttm.copy()
                        combined_export = pd.concat([combined_dl, ttm_display.T.add_prefix(\"TTM_\")], axis=0, sort=False)
                    else:
                        combined_export = combined_dl
                    csv_bytes = combined_export.to_csv().encode()
                    st.download_button("Download Quarterly & TTM CSV", data=csv_bytes, file_name=f"{selected}_quarterly_ttm.csv", mime="text/csv")
                except Exception as e:
                    if st.session_state.get("debug_mode", False):
                        st.error(f"Download prepare error: {e}")
        # ---------- END NEW: Quarterly & TTM Analysis Tab ----------
