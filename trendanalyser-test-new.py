...
# - FIX: ValueError in DataFrame creation (added .flatten())
...
def analyze_single_ticker(ticker: str, fy_start: int, force_scrape: bool = False, debug: bool = False, include_other_income: bool = False):
    ...
    if qr_df is not None and not qr_df.empty:
        ...
        if profit_row_idx_q:
            ...
            if len(profit_q) >= 4:
                ...
                # FIX: Use quarter-end close + convert to Timestamp
                price_q = price_df["Close"].resample("Q").last().round(2)
                common_ttm = ttm_series.index.intersection(price_q.index)
                if len(common_ttm) >= 2:
                    common_ttm = sorted(common_ttm)
                    has_ttm = True
                    ttm_clean = ttm_series.loc[common_ttm]
                    qtr_end_price_ttm = price_q.loc[common_ttm]
                    ttm_1d = np.array(ttm_clean).flatten()
                    price_1d_t = np.array(qtr_end_price_ttm).flatten()
                    ratio_ttm_1d = np.round(price_1d_t / ttm_1d, 4)
                    merged_ttm = pd.DataFrame(
                        {
                            f"{ttm_label} TTM (Cr)": ttm_1d,
                            "Quarter End Price": price_1d_t,
    ...
    return {
        ...
        "price_series": price_df.reset_index(),  # Updated to include full OHLC data
        # TTM additions
        "has_ttm": has_ttm,
        "ttm_label": ttm_label,
...
if "summary_df" in st.session_state:
    ...
    if selected and "analysis_results" in st.session_state and selected in st.session_state.analysis_results:
        ...
        st.markdown("## Purple Profit & Red Price")
        # FUNDAMENTAL VS PRICE CHART (distinct colors)
        chart_df_p = res["merged_profit"][[f"{res['profit_label']} (Cr)", "FY End Price"]].reset_index()
        ...
        line_price = base_p.mark_line(color="#FF0000", strokeWidth=3).transform_filter(alt.datum.Metric == "Price").encode(
            y=alt.Y("Value:Q", title="FY End Price")
        )
        ...
        if not res["merged_sales"].empty:
            ...
            line_price_s = base_s.mark_line(color="#FF0000", strokeWidth=3).transform_filter(alt.datum.Metric == "Price").encode(y=alt.Y("Value:Q", title="FY End Price"))
        ...
        with tabs[0]:
            ...
            # FULL HISTORIC PRICE CHART (candlestick with filters)
            price_series = res["price_series"]
            price_series["Date"] = pd.to_datetime(price_series["Date"])
            if not price_series.empty:
                min_date = price_series["Date"].min().date()
                max_date = price_series["Date"].max().date()
                col1, col2 = st.columns(2)
                with col1:
                    start_date = st.date_input("Start Date", value=min_date, min_value=min_date, max_value=max_date, key="start_date_op")
                with col2:
                    end_date = st.date_input("End Date", value=max_date, min_value=min_date, max_value=max_date, key="end_date_op")
                filtered_series = price_series[(price_series["Date"].dt.date >= start_date) & (price_series["Date"].dt.date <= end_date)]
                # Candlestick chart
                body = alt.Chart(filtered_series).mark_bar().encode(
                    x=alt.X("Date:T", title="Date"),
                    y=alt.Y("Open:Q", title="Price (Rs.)"),
                    y2="Close:Q",
                    color=alt.condition("datum.Open <= datum.Close", alt.value("#00FF00"), alt.value("#FF0000")),  # Green up, Red down
                    tooltip=["Date:T", "Open:Q", "High:Q", "Low:Q", "Close:Q"]
                )
                wicks = alt.Chart(filtered_series).mark_rule().encode(
                    x=alt.X("Date:T"),
                    y="Low:Q",
                    y2="High:Q",
                    color=alt.condition("datum.Open <= datum.Close", alt.value("#00FF00"), alt.value("#FF0000")),
                    tooltip=["Date:T", "Open:Q", "High:Q", "Low:Q", "Close:Q"]
                )
                chart_price = alt.layer(body, wicks).properties(
                    width="container",
                    height=350,
                    title="Historical Stock Price (Candlestick)"
                ).interactive()
        ...
        if res["has_sales"]:
            with tabs[1]:
                ...
                # FULL HISTORIC PRICE CHART (candlestick with filters) - repeated for Sales tab
                price_series = res["price_series"]
                price_series["Date"] = pd.to_datetime(price_series["Date"])
                if not price_series.empty:
                    min_date = price_series["Date"].min().date()
                    ...
                    col1, col2 = st.columns(2)
                    with col1:
                        start_date = st.date_input("Start Date", value=min_date, min_value=min_date, max_value=max_date, key="start_date_sales")
                    with col2:
                        end_date = st.date_input("End Date", value=max_date, min_value=min_date, max_value=max_date, key="end_date_sales")
                    filtered_series = price_series[(price_series["Date"].dt.date >= start_date) & (price_series["Date"].dt.date <= end_date)]
                    # Candlestick chart
                    body = alt.Chart(filtered_series).mark_bar().encode(
                        x=alt.X("Date:T", title="Date"),
        ...
        if res["has_ttm"]:
            ...
            with tabs[ttm_tab_idx]:
                ...
                # TTM Chart
                chart_df_t = res["merged_ttm"][[f"{res['ttm_label']} TTM (Cr)", "Quarter End Price"]].reset_index()
                ...
                line_price_t = base_t.mark_line(color="#FF0000", strokeWidth=3).transform_filter(alt.datum.Metric == "Price").encode(
                    y=alt.Y("Value:Q", title="Quarter End Price")
                )
                ...
                if res.get("price_series_q") is not None:
                    ...
                    price_q_ser.columns = ["Date", "Close"]
                    price_q_ser["Date"] = pd.to_datetime(price_q_ser["Date"])
                    chart_price_q = alt.Chart(price_q_ser).mark_line(color="#3498db", strokeWidth=3).encode(
                        x=alt.X("Date:T", title="Quarter End"),
                        y=alt.Y("Close:Q", title="Quarter End Price (Rs.)"),
                        tooltip=["Date:T", "Close:Q"]
                    ).properties(width="container", height=250, title="Quarter End Stock Price").interactive()
                ...
                if res.get("has_sales_ttm", False):
                    ...
                    # TTM Sales Chart
                    chart_df_st = res["merged_sales_ttm"][[f"{res['sales_label_q']} TTM (Cr)", "Quarter End Price"]].reset_index()
        ...
        if not res["merged_sales"].empty:
            combined = combined.join(res["merged_sales"].drop(columns="FY End Price", errors="ignore"), how="left")
        if not res["merged_ttm"].empty:
            combined = combined.join(res["merged_ttm"].drop(columns="Quarter End Price", errors="ignore"), how="left", lsuffix="_annual")
